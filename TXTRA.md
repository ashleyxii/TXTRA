...
#### コロン構文の判定規則 (update 1.0.3)
コロンによる属性・コンテナ判定は、**コロン直後に半角空白・タブがある場合（`: `）**、または **コロンが行末にある場合（`:`）** のみを対象とします。
これにより、時刻（`13:54`）や URL（`https://...`）、ポート番号（`:8012`）などのコロンを含む地の文が誤ってキー・バリューとして構造化されるのを安全に防止します。

#### スカラー値付きキーの自動コンテナ昇格 (update 1.0.3)
`Key: Value` のように既にスカラー値を持つノードの配下に、後からインデントされた子要素がネストされた場合、パーサーは既存の値を自動的に無名ノード `{ node: '_', data: ['Value'] }` にラップしてコンテナ化します。
「代表値＋追加属性」という人間の自然な思考の殴り書きをそのまま許容し、整然とした木構造へ安全に昇格させます。
